...
public interface MemberMapper {
   ...
   // memberIdCount()  재사용 
   //2=>비밀번호 검색 
   @Select("SELECT pm.userId,userName,userPwd,enable,authority "
		  +"FROM projectMember pm,projectAuthority pa "
		  +"WHERE pm.userId=pa.userId "
		  +"AND pm.userId=#{userId}")
   public MemberVO memberLogin(String userId);
   
   @Select("SELECT pm.userId,userName,userPwd,sex,email,phone,addr1,addr2,enable,authority "
			  +"FROM projectMember pm,projectAuthority pa "
			  +"WHERE pm.userId=pa.userId "
			  +"AND pm.userId=#{userId}")
	public MemberVO memberInfo(String userId);
   
   @Select("SELECT pm.userId,userName,sex,email,phone,addr1,addr2,enable,authority "
		  +"FROM projectMember pm,projectAuthority pa "
		  +"WHERE pm.userId=pa.userId "
		  +"AND pm.userId=#{userId}")
