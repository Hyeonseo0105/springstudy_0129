      ...
      <div class="col-md-3" v-for="vo in food_list">
       <a :href="'../food/detail.do?fno='+vo.fno">
	    <div class="thumbnail">
	        ...
	        <div class="caption">
	          <p style="font-size:11px">{{vo.name}}</p>
	        </div>
	    </div>
	   </a>
	  </div>
    ...
    <div class="row">
      <div class="text-center">
        <ul class="pagination">
          ...
          <li v-for="i in range(startPage,endPage)" :class="curpage===i?'active':''"><a class="lick" @click="pageChange(i)">{{i}}</a></li>
...
<script>
  let app=Vue.createApp({
	  // 멤버변수 설정 => Model => 변경된 내용을 View(mount에 저장된 HTML)로 전송	  
	  data(){
		  return{
			  food_list:[],
			  curpage:1,
	  ...
	  // ViewModel => 기능처리 ===> Model에 있는 데이터값(상태)에 변경  
	  mounted(){
		  // 시작과 동시에 데이터 처리
		  this.dataRecv()
	  },
